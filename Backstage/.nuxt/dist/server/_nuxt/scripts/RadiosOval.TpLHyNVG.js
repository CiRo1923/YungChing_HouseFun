  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(props.options, (item, index) => {
        ...
        ], "m-form-element flex h-full w-full cursor-pointer items-center justify-center text-[--gray-666] transition-colors duration-300 tm:gap-x-[3px] tm:px-[10px] p:gap-x-[5px] p:px-[12px]"])}"><input type="radio"${ssrRenderAttr("name", props.name)}${ssrIncludeBooleanAttr(ssrLooseEqual(unref(selected), item[unref(config).schema.value])) ? " checked" : ""}${ssrRenderAttr("value", item[unref(config).schema.value])} class="${ssrRenderClass([unref(setClass).type, "m-form-type absolute left-[-99999px]"])}">`);
...
//# sourceMappingURL=RadiosOval.TpLHyNVG.js.map
