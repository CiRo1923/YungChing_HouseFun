import { mergeProps, useSSRContext, defineComponent, toRef, resolveDynamicComponent, h, ref, reactive, computed, toValue, unref, isRef, watch, provide, readonly, inject, watchEffect, nextTick, shallowRef, warn, getCurrentInstance } from "vue";
import { ssrRenderAttrs, ssrInterpolate } from "vue/server-renderer";
import { u as unicodLength } from "./_prototype.DwbhdnQa.js";
import { all } from "@vee-validate/rules";
const _sfc_main = {
  __name: "mErrorMessageElem",
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<small${ssrRenderAttrs(mergeProps({ class: "m-error-message block text-[--orange-e646]" }, _attrs))}>${ssrInterpolate(props.message)}</small>`);
    };
  }
};
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/buy/mErrorMessageElem.vue");
...
defineRule("required", (value, object, elem) => {
  return value === null || value === void 0 || value.length === 0 ? replaceMessage(elem, object) : object.type && object.type === "boolean" && value !== true ? object.errorMessage : true;
});
...
defineRule("halfWidth", (value, object) => {
  const isObject2 = !!object.exception;
  const exception = isObject2 ? value.replace(new RegExp(`\\${object.exception}`, "g", "")) : value;
  return value && /[^x00-xff]/g.test(exception) ? isObject2 ? object.message : object[0] : true;
});
...
//# sourceMappingURL=mForm.css.Kz_kpQ5n.js.map
