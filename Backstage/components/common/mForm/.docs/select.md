...
`maxItems` 控制 dropdown 超過指定項目數時的可視高度。高度會寫在 `.m-form-dropdown-body`，卷軸也由 `.m-form-dropdown-body.scrollbar.--y` 負責。
...
外層 `.m-form-dropdown` 高度會再依照 container 內容重新計算，因此 header / footer 不會被算進 body 的捲動區。
