    ...
    JSON.stringify({
      hookSpecificOutput: {
        ...
        additionalContext:
          ...
          '沒有 sm / md / lg / xl、沒有 font-sans,而 boxShadow 一個 preset 都沒有(連 shadow-sm 也不存在,' +
          '陰影一律走原生 box-shadow + module 變數),字級一律 arbitrary value;' +
          '斷點是 m / t / p / tm / pt / pMin / pMax / mLandscape,全部是 raw media query。' +
          '這些由規則 6 守門(checkTailwindTheme)。' +
          'plugin 另外提供 text-hexa / bg-hexa / border-hexa / divide-hexa(語法 bg-hexa-[--black,0.7]),' +
          '這四個在**這個專案仍是合法用法**(Official 那邊已淘汰)。\n' +
          '同一個 @apply 裡字級與文字顏色**可以併存**(text-[length:--a] text-[--b] 兩個都會出現),' +
          '不要為此退回原生 color: —— 規範舊版寫反過,已依 npx tailwindcss 的實際產物更正' +
          '(產出 font-size 與 color 兩條,2026-08-31 兩個專案各自重驗過)。\n' +
