...
const onBuyRealEstateSearch = async (done) => {
  const result = await onApiPostBuyRealEstateSearch(2)
...
const listDraft = useAsyncData('list-draft', () => onBuyRealEstateSearch(), {
  watch: [page],
})
...
await Promise.all([
  ...
  callOnce('list-plan-aggergate-draft', () => onApiGetVasCommonPlanAggregate()),
  callOnce('list-case-aggregate-draft', () => onApiGetBuyRealEstateCaseAggregate()),
  callOnce('available-plans-draft', () => onApiGetVasPublishAvailablePlans()),
  callOnce('golden-planList-draft', () => onApiGetVasGoldenGetPlanList()),
  callOnce('comments-search-draft', () => onApiGetBuyCommentsSearchCommentFilter()),
...
<template>
  ...
  >
    ...
    <PageBuyListTabDefaultOval>
      <PageBuyListFilterDraft @search="onBuyRealEstateSearch" />
    </PageBuyListTabDefaultOval>
    <PageBuyListContent
