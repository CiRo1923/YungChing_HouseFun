// lint-project-name-exempt: 這支檔案是「專案名稱」那條規則的資料來源 ——
// 名稱必須明明白白寫在這裡,規則才知道要抓什麼。整份跳過那條檢查。
//
...
//    只是從此不再檢查任何東西。
//
// **每個專案只做一件事:把下面既有的項目改成自己專案的值。**
//    不要自己新增一項、刪掉一項,也不要改名 —— 項目有哪些由來源決定,
//    規則讀什麼,這裡才有什麼。
//
//    自己多加的那一項沒有任何規則會讀它:不會有錯誤訊息,什麼都不會發生,
//    而下一個人看到它會以為某條規則吃這個值。刪掉或改名更直接 ——
//    規則 import 的名稱不存在,那條規則整條失效,或是工具載入時就壞掉。
//
//    需要一個新的設定項時,那代表規則本身要改(有一段判準寫死在規則裡,
...
/**
 * 後端 api 的規格文件放哪 —— 用來認出「這個欄位名是後端給的,還是前端自己掛上去的」。
 *
 * **每個專案都用同一個資料夾名**,底下的 `.json` 全部算數 ——
 * 一個專案打好幾個服務時(各自有 baseURL 與 token),每個服務一份,不必合併成一支。
 * 路徑相對專案根目錄;**沒有規格文件的專案留空字串**,吃它的那條規則就整條略過。
 *
 * 認得 OpenAPI / Swagger 的 json,也就是後端那邊直接匯出的那一份。
 * **這份檔案是人放進來的**,沒有任何自動更新 —— 後端改了規格就重新匯出一次蓋掉。
 * 讓它在開發時自動抓的話,規則吃的就是一份會自己變動的東西:
 * 同一份程式碼今天過、明天不過,而看到訊息的人不知道是誰改的。
 *
 * 其他格式(試算表、文書檔)不支援:欄位名寫在哪一欄、哪一列每一份都不一樣,
 * 等於每個專案各寫一支解析;json 的形狀是固定的。
 *
 * 文件過期時(後端加了欄位但沒有重新匯出)的徵狀是:那個新欄位被報成
 * 「前端自訂欄位要加底線」。那時要做的是重新匯出文件,不是照著加底線 ——
 * 加了之後那個欄位就再也對不上後端。
 */
export const API_SPEC_DIR = '.api.swagger'

/** 共用元件 */
export const COMPONENTS_DIR = 'components'

/**
 * `.vue` 的 import 分組順序 —— 由上而下就是該寫的順序。
 *
 * 為什麼要有固定順序:一支檔案開頭那十幾行 import,每個人寫的順序都不一樣的話,
 * 要找「這支有沒有載入某個樣式」得整段看完。分組固定之後,看第幾段就知道找什麼。
 *
 * 每一組的 `match` 是比對 import 路徑的正規表示式(字串形式)。
 * 由上往下比對,第一個命中的就是它的組;都沒命中的算「其他」,排在最後。
 *
 * **第一組固定是樣式。** 元件沒有載入這一組的任何一支就會被報違規 ——
 * 元件的樣式要由元件自己 import,少了那一行,它被放進別的頁面時樣式不會跟著來。
 *
 * **這幾條只管組與組之間的先後,不管同一組裡面怎麼排。**
 *    組內的順序另有規則(例如樣式那一組,變數檔要排在版型檔之前),
 *    兩邊混在一起的話,同一行會同時被兩條規則指指點點,而修法還不一樣。
 *
 * 各專案的 alias 與資料夾命名不同,所以這份清單是設定 ——
 * 比對式寫死在規則裡的話,換一個 alias 命名的專案就會把每一支都報成順序錯。
 */
 ...
 * 掛在版面上的系統元件、版型。
 *
 * 系統元件那一層再分兩種,判準是「換一個專案還會不會用到」:
 * 每個專案都會有的(警告、確認、請求中的提示、自訂彈窗)放共用那一疊,
 ...
 * 規則的提示訊息會跟著指向正確的檔案。
 *
 * **集中目錄還沒有共用變數檔的專案留空字串**,提示訊息就只講「搬到模組自己的
 * Variables 檔」,不再指一個不存在的位置。樣式跟著元件走的專案很容易走到這一步:
 * 每一支樣式都對得上某個元件,集中目錄最後就空了。
 *
 * 填一個不存在的檔名的話,照著提示做的人會建出一支檔案 ——
 * 而那支檔案裡只有一組變數、放在一個沒有別的東西的目錄,
 * 下一個人看到不知道為什麼它在那裡。真的出現第一支跨模組共用的變數時再填,
 * 那時才知道它該叫什麼、該跟誰放在一起。
 */
...
/**
 * 每一個 `@screen` 區塊裡,父層可傳入的級距要列出哪幾種前綴。
 *
 * 級距 class 由使用端傳進來(`p:--px-20`、`tm:--py-8`),而 CSS 這邊要為
 * **每一個會命中的斷點**各寫一次選擇器。少列一種的後果:使用端那樣寫了,
 * 在那個斷點下卻沒有任何效果 —— 畫面上是「這個間距沒生效」,
 * 而那一行 class 看起來完全正常,很難聯想到是樣式檔少列了一個選擇器。
 *
 * 形狀是「`@screen` 的名字 → 在那個區塊裡會命中的前綴」。
 * 一個前綴會出現在它涵蓋的每一個斷點底下:涵蓋平板與手機的那種前綴,
 * 在平板與手機兩個區塊都要列。
 *
 * **無前綴的寫法不必列在這裡** —— 那代表「所有斷點都套用」,每一個區塊都要有,
 * 規則直接要求,不必每個專案重寫一次。
 *
 * 這一項與上面的 BREAKPOINTS 不是同一件事,兩者都要填:
 *
 *   BREAKPOINTS          變數**名字**裡的那一段(`--popup-pc-w` 的 `pc`)
 *   BREAKPOINT_SCREENS   CSS 的 `@screen` 名字與 class 前綴(`@screen p`、`p:--px-20`)
 *
 * 多數專案這兩套名字不一樣(一個求好讀,一個求好打),所以分成兩項。
 *
 * **不做響應式的專案留空物件**,這條規則就整條略過 ——
 * 檢查結果旁邊會列出「這條這次沒有作用」,不會安靜地跳過。
 */
export const BREAKPOINT_SCREENS = {
  p: ['p', 'pt'],
  t: ['t', 'pt', 'tm'],
  m: ['m', 'tm'],
}
 ...
 */
/**
 * 可以在 store 裡直接呼叫的取值函式 —— 讀設定、讀環境的那幾支。
 *
 * store 只放狀態宣告,行為一律進 actions。但有一種呼叫不是行為:
 * **把執行期的設定讀出來,拿它組初始值**(依環境決定預設帶不帶測試帳號那種)。
 * 那個值在 store 建立的當下就要有,搬進 actions 的話,
 * 預設值就不能再是一份靜態宣告了 —— 而「預設值集中成一份 readonly」
 * 正是另一條規則要求的形狀,兩條會打架。
 *
 * 各框架讀設定的方式不一樣:有的是一支函式(Nuxt 的 useRuntimeConfig),
 * 有的直接讀建置時注入的變數(Vite 的 import.meta.env,那種不是呼叫,本來就不會被報)。
 * 所以這份清單由專案填,規則不寫死任何框架的名字。
 *
 * **只填「讀出來就結束」的那幾支。** 會打 api、會改狀態的一律不要列 ——
 * 那是行為,列進來等於讓那條規則對這個專案失效,而且看不出來。
 */
export const STORE_SETUP_CALLS = ['useRuntimeConfig']

/**
 * pinia 實例本身是哪一支檔案。
 *
 * 那一支只做一件事:建立狀態管理的實例並匯出。它不是一個 store,
 * 所以「檔名要對得上頁面資料夾」那幾條不該管它。
 *
 * 各專案放的位置與檔名不同(有的叫 index.js,有的另外取名),
 * 所以由專案填。填錯或留空的話,那一支會被當成一般的 store 檢查 ——
 * 報出一整批「對不上頁面資料夾」,而那全是誤報。
 *
 * 本專案留空:實例由 @pinia/nuxt 這個模組建立,stores 底下沒有那種檔案。
 */
export const STORE_INSTANCE_FILE = ''
 ...
 * 端點裡不計入函式名的固定段落。
 *
 * 有些專案的端點前面固定帶幾段,那幾段對讀的人沒有意義:
 *
 *   api/v1/buy/list   →  函式名要的是 buy/list,不是 apiGetV1BuyList
 *
 * 不排掉的話,每一支的期望名都多出那一段,而那一段每一支都一樣。
 *
 * **變數插值(`${version}` 那種)不必列在這裡** —— 規則本來就不計入它們:
 * 那種段落的值執行期才決定,不是端點的語意。這份清單是給**寫死的字面**用的。
 *
 * **更好的做法是把固定前綴併進連線設定的 baseURL**,端點只留真正的路徑 ——
 * 那樣函式名與端點一眼對得起來,也不必維護這份清單。
 * 這一項是給「前綴改不動」的情況(後端已經定案、或那段本來就該留在端點裡)。
 *
 * 比對不分大小寫;沒有這種情況就留空陣列。
 */
...
/**
 * 彈窗元件在畫面上寫出來的標籤名,與它內建的那幾個 id。
 *
 * 全站同一時間只有一個彈窗可見:每個實例靠自己的 id 與「現在開著的是誰」
 * 比對,相同才顯示。所以宣告端寫的 id 與開啟時傳的 id 必須一模一樣。
 *
 * 對不上時**沒有任何人會出聲**:畫面不會報錯(不認得的屬性框架就是靜靜忽略)、
 * 建置不檢查、程式的靜態檢查也管不到畫面區段裡的屬性名。
 * 要真的按下那個按鈕才會發現彈窗打不開。
 *
 * POPUP_TAGS  畫面上寫出來的標籤(自動注入的話就是那個名字);
 *             一個專案可能有好幾種(共用的那支、再包一層的那支)
 * BUILTIN_IDS 不必自己宣告、由開啟函式寫死的那幾個 —— 那幾個要有對應的實例,
 *             少一個的話那一類彈窗全站都開不起來
 *
 * 兩項都留空的話,「彈窗的 id 有沒有配對」那條規則會整條略過。
 */
export const POPUP_TAGS = ['CommonMPopup', 'CommonCustomPopup']

export const BUILTIN_POPUP_IDS = ['alertSystem', 'confirmSystem', 'apiPromiseSystem']
...
export const PROJECT_NAMES = [
  'House Fun', // 本專案
  'Yung Ching', // 集團名稱 —— 套件設定與資料夾名裡會出現
]

/**
 * 規範工具的來源專案叫什麼名字。
 *
 * 這套工具在來源那邊長出來,再整套複製到別的專案。兩邊能做的事不一樣:
 *
 *   來源    規則與設定一起長,新增設定項是正常的
 *   其他專案 只把既有的設定項改成自己的值,不新增、不刪、不改名
 *
 * 判斷方式是拿這個名字與上面的 PROJECT_NAMES 比對:**對得上就是來源。**
 * 每個專案安裝時本來就要把 PROJECT_NAMES 換成自己的名字(不換的話,
 * 「不寫死專案名稱」那條會去抓一個與它無關的字),換掉之後兩者自然對不上,
 * 就不再是來源。
 *
 * 來源之後換人時改這裡:新的來源專案把這裡填成自己的名字,
 * 舊的那邊就自動退回一般專案。比對與 PROJECT_NAMES 用同一套寫法,
 * 空白、底線、連字號、大小寫的差異都算同一個名字。
 */
export const SOURCE_PROJECT_NAME = 'F2E'
 ...
 *
 * **這份清單列的是「tailwind 有哪幾類、各類的內建值叫什麼」,與專案無關**,
 * 所以換專案不必改;升級 tailwind 大版本、預設值有增減時才要跟著調。
 * 每一類都列齊,不要因為「這個專案沒有覆寫那一類」而不列 ——
 * 少列一類,那一類就從來不會被檢查,而且沒有任何徵兆。
 *
 * **這個專案實際覆寫了哪幾類、各類還剩哪些值,兩者都不寫在這裡** ——
 * 規則直接讀專案的 tailwind 設定算出來(見 STYLE_CONFIG_FILES):
 * 沒有被整組覆寫的類別完全不檢查,被覆寫的才提醒,而訊息裡「可用的是…」
 * 也是從那裡讀的。在這裡抄一份的話,改了 tailwind 設定忘了改這裡,
 * 規則會開始講錯話(指著一個已經改掉的名字叫人去用),而且不會有任何徵兆。
 */
...
/**
 * 由工具自動產生、會跟著原始碼一起進版控的檔案(路徑從專案根算起)。
 *
 * 典型的是元件與 store 的自動注入清單:建置工具在背景監看原始碼目錄,
 * 一看到新的檔案就把它登記進去。
 *
 * **規則自己的驗證會在元件目錄底下實際建出探測檔**,那些監看的工具會看到,
 * 於是把探測元件也登記進來;探測檔刪掉之後,那幾行留在清單裡,
 * 指向一個不存在的檔案。不清掉的話會跟著 commit 擴散出去,
 * 而看到的人不知道那是什麼 —— 那幾個名字在專案裡根本找不到。
 *
 * 驗證結束時會把這幾支檔案裡「提到探測名字」的行清掉,其餘內容一個字都不動。
 *
 * **只列會進版控的那幾支。** 建置產物(打包出來的東西)不必列 ——
 * 那些本來就不進版控,也不會有人看到裡面多了什麼。
 *
 * 沒有這種檔案的專案留空陣列。
 */
...
/**
 * 「不用 emoji 與裝飾符號」那一條不檢查的目錄(路徑從專案根算起)。
 *
 * 有些目錄裡的文字不是這個團隊在寫:從別處整包複製進來的元件、
 * 產生器吐出來的檔案、外部平台要求原樣保留的內容。那些地方出現圖示
 * 不是寫的人選的,報出來也沒有人能改 —— 而一條一直報「改不了的東西」的規則,
 * 最後會連同真正該改的那幾筆一起被略過。
 *
 * **比對的是路徑,不是資料夾名。** 寫 `components/vendor` 只排除那一處;
 * 別的地方另一個同名資料夾照常檢查。要排除兩個位置就列兩筆。
 *
 * **只影響這一條。** 那幾層的其他檢查(專案名稱、絕對路徑、讀者不必跳頁,
 * 以及所有程式寫法的規則)照常適用 —— 要整層都不檢查的是另一回事,
 * 那種目錄根本不該放在原始碼裡面。
 *
 * 排除的範圍會列在檢查結果的開頭,不會安靜地少檢查一塊。
 * 多數專案不需要這一項,留空陣列即可。
 */
export const PLAIN_TEXT_EXCLUDED_DIRS = ['.dev']

/**
 * 不是原始碼、但仍要檢查的目錄 —— 檢查工具、給 AI 讀的規範(含它的說明文件)、
 * commit 前的檢查、建置期的外掛與設定。
 *
 * 這些目錄裡的檔案不是原始碼,api、store、頁面那幾類規則不適用於它們;
 * 但文字怎麼寫那幾條照樣要守(不寫絕對路徑、不用裝飾符號、讀者不必跳頁)。
 *
 * **其中規範工具自己的那幾個會整批複製到下一個專案**,所以「不寫專案名稱」
 * 這條只管這裡 —— 專案自己的建置外掛列進來不會有壞處,那些檔案本來也不需要
 * 寫出專案名稱。
 *
...
/**
 * 規範系統自己的說明文件 —— 有哪些規則、怎麼裝到新專案。
 *
 * 這一層要講齊每一條規則:規則寫在程式裡,而看的人是從這裡知道
 * 「這套工具在管什麼」的。少講一條的話,那條規則照樣在擋,
