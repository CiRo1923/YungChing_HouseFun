...
//
//   node .tools/lint/diff-output-project.mjs --collect <輸出檔>   # 把 .output 的 CSS 收成一份
//   node .tools/lint/diff-output-project.mjs <舊> <新>            # 比對兩份
//
...
//      npm run build && node .tools/lint/diff-output-project.mjs --collect /tmp/old.css
...
if (args[0] === '--collect') {
  if (!args[1]) {
    console.error('✗ 用法:node .tools/lint/diff-output-project.mjs --collect <輸出檔>')
    process.exit(1)
  }
  onCollect(args[1])
  ...
  console.error(
    '用法:\n' +
      '  node .tools/lint/diff-output-project.mjs --collect <輸出檔>   # 收集 .output 的 CSS\n' +
      '  node .tools/lint/diff-output-project.mjs <舊> <新>            # 比對兩份'
  )
  process.exit(1)
}
