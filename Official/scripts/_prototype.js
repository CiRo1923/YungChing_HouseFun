...
// 同時執行多支 api,會等全部回傳。
//
// 失敗時不往上拋,只留一行 log 讓畫面繼續 —— 所以這一支用在**已經看得到畫面之後**
// 的取資料:換頁重打、切換排序、使用者按下某個按鈕。那時畫面上本來就有東西,
// 為了其中一支失敗把整頁換成錯誤頁,使用者反而失去原本看得到的內容。
//
// **進入頁面時的第一次取資料不要用這支**,那時要的是相反的行為:
// 用 Promise.all 讓失敗往上拋,伺服器端才會回 500。吞掉的話會送出一個
// 內容是空的、狀態卻是 200 的頁面,而搜尋引擎會把那一份當成真的沒內容收下去。
