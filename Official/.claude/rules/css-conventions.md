...
| ✗ | ✓ |
|---|---|
| 「與 XXX 的差異:YYY 有三個 preset」 | 「移植時要重新對照那邊的 config —— **本專案**有三個 preset」 |
| 「2026-08-31 清完存量(A 專案 5 處、B 專案 1 處)」 | 「2026-08-31 清完**本專案**的存量(5 處)」 |
...
不要提對方是誰。例如陰影那條 —— 寫「本專案的 `tailwind.extend.js` 有三個 preset,
所以只抓內建的 key;若對方的 extend 不放陰影,要改成抓任何 `shadow-*`」。
...
> 改方向當天把一支整份採用巢狀寫法的變體檔改成頂層集中(6 筆違規)。
