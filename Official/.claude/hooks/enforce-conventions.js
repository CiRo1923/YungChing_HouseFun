...
/* 規則 11:規範複本一律整包同步,不要自己改寫

  .claude/rules/、.claude/skills/、.tools/lint/ 這幾處是另一份規範的複本。
  它們的價值在於「兩邊真的一樣」——一旦這邊自己改過,就再也說不出
...
const onCheckSyncScope = (filePath) => {
  ...
  return (
    '[sync] 這支檔案是規範複本。同步的做法固定成三件事,動手前先確認:' +
    '(1) 每一次都重新讀來源目錄的每一支檔案 —— 不靠記憶、不靠上一輪的比對結果、' +
    '也不看修改時間判斷「這支沒動過」。' +
    '(2) 整包覆蓋,取用之後不要改名、不要把這邊多的函式接回去、不要補判斷讓它適用這邊 ——' +
    '接不上是要回報的事實,不是就地解決的問題。' +
    '(3) 只有設定值可以跟著這個專案調整,而且改之前要先問過開發者;' +
    '判斷邏輯、名稱、檔名的差異一律先列出來問,不要自己選。'
  )
}
