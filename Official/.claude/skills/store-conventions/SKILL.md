...
規則 `storeDefaultClone`(擋)。兩件事同時成立才報:`apiDefault` 的某一層有陣列,
**而且**全案有人用展開一層的方式還原那一層。訊息會指出是哪一支檔案的哪一行。

還原處都已經深拷貝、或根本沒有人還原那一層,都不報 —— 寫對了就消得掉。
專案沒有那支深拷貝的共用函式時(設定的 `DEEP_CLONE_HELPER` 留空),
這條整條略過:分不出哪一處已經寫對,報出來的每一筆都沒有修法可以照著做。

**存檔時會自動改好** ——
從 `apiDefault` 還原的地方換成深拷貝,那支函式的 import 也一起補上。

`readonly` 是**深層的**:底下每一層都是唯讀的。展開一層(`{ ...apiDefault.x }`)
只複製了最外面那一層,**陣列與巢狀物件拿到的仍然是同一個唯讀的東西**。

後果是「還原之後那個陣列不能寫」:

```js
apiData.value = { ...apiDefault.x }   // 錯誤：ItemList 仍然是唯讀的那一個
apiData.value.ItemList.push(item)     // 加不進去,長度還是 0
```

**開發時看得到警告,正式版沒有任何徵兆** —— 畫面上是「按了新增卻沒反應」,
而那一行 `push` 看起來完全正常。checkbox 群組綁陣列時特別容易遇到:
`v-model` 是就地增刪,不是整組換掉。

**正確的寫法是深拷貝**,拷貝出來的每一層都是新的、可以寫:

```js
apiData.value = onDeepClone(apiDefault.x)
```

專案沒有自己的深拷貝工具時,`structuredClone(toRaw(apiDefault.x))` 也可以 ——
`toRaw` 要先拿掉那層唯讀代理,少了它會直接拋錯。

**全部是單純值(字串、數字、null)的那一層,展開就夠了。** 判斷方式是
「這一層裡有沒有陣列或物件」——有就深拷貝,沒有就展開。
不確定的時候一律深拷貝:多拷貝一層沒有壞處,漏了則是沒有徵兆的失效。

### 一層裡有兩種生命週期的欄位時

有一種流程是「先指定要操作誰,再讓使用者填細節」:呼叫端先把操作對象寫進
`apiData`,接著重設上一次的選取,然後靠那個對象去打 api。同一層裡因此有兩種欄位:

| 欄位 | 誰決定 | 重設時 |
| --- | --- | --- |
| 操作對象(哪一筆資料、哪一個項目) | 呼叫端 | 要留著,後面的 api 靠它 |
| 使用者的選擇 | 畫面上的操作 | 清掉 |

**先重設、再指定這次要操作誰** —— 重設的意思就是「回到初始」,
在它之後才寫入這一次的對象,順序與語意一致,也不需要任何特例:

```js
onResetXxx()                    // 先回到初始
save.apiData.itemId = itemId    // 再指定這次操作誰
await onApiGetDetail()
```

順序不能動的時候(重設本身就要用到那個對象),**整組還原之後把要留的帶回去**:

```js
const { itemId } = save.apiData

save.apiData = { ...onDeepClone(apiDefault.xxx.save), itemId }
```

兩種寫法規則都放行。**不要改成逐欄位清掉那幾個要清的** ——
那樣之後新增一個欄位時,要記得在重設那邊多寫一行,漏了不會報錯,
只會在下一次進到這個流程時帶著上一次的值;而且「哪幾個是刻意留著的」
要比對兩份清單才看得出來,寫成上面那樣則是直接寫在那一行。

`apiDefault` 的語意不因此改變:**它是這一層的完整初始值**,
store 的初始宣告(`apiData: { ...apiDefault.xxx }`)讀的是同一份。
改成「只放重設要還原的那幾個」的話,初始宣告就缺欄位、得另外補一份 ——
預設值又變成兩個地方,那正是這一條要避免的事。
