...
name: import-alias
summary: import 路徑
description: 本專案的 import 路徑規範。當需要在原始碼新增或修改 import(.js/.mjs/.ts/.vue),或審查、整理既有 import 路徑時使用。規則:離開自己資料夾的相對路徑(含 ..)一律改用建置設定 resolve.alias 定義的 @ 系列 alias;一次收一整批檔案的呼叫(import.meta.glob)吃的也是路徑,同樣要用 alias —— 那種路徑找不到檔案不會報錯,只會收到空的一批。
---
...
建置工具有一種「把符合條件的檔案一次全部收進來」的寫法
(`import.meta.glob('…/*.vue')`),它吃的也是路徑,**同樣要用 alias**。

那裡的路徑寫成相對的話,後果比一般 import 嚴重:一般 import 找不到檔案會
在建置時報錯,而這種呼叫**找不到就回一批空的,不會有任何訊息** ——
用它的地方靜靜地拿不到東西,畫面上像是資料沒出來,而那一行看起來完全正常。
