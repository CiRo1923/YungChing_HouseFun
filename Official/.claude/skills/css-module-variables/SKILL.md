...
summary: css 模組變數
description: 本專案的 CSS 模組變數分層規範。當需要在模組樣式(元件資料夾底下的樣式子資料夾,或跨模組共用的集中目錄)新增或調整樣式、加入 --px-XX / --py-XX / --h-XX / --rounded-XX 這類可由父層覆寫的尺寸級距,或審查既有模組 css 時使用。規則:同一屬性有兩個以上級距值的覆寫 class 一律定義在 ***Variables.css(單一值可留在模組 css);作用域依實際共用範圍決定,只有這個模組用的放該模組的 Variables 檔、跨模組共用的放共用 variables.css。
---
